...
def home_page():
    ...
    """, unsafe_allow_html=True)
    st.markdown("---")
    st.info("Select a section from the sidebar to explore the system.", icon="👈")
...
├── Building Approval Fee Collection
├── Property Development Revenue Tracking
├── Compounding Fee Collection
├── Cross-Vertical Investment Strategies
├── ROI Maximization Across All Functions
└── Integrated Profit Center Management
            """, language='python')
    with cols[1]:
        with st.container():
...
├── Prime Land Identification for MDDA
├── Building Approval Hotspot Analysis
├── Infrastructure Priority Matrix
├── Property Market Demand Assessment
├── Competitive Pricing Strategy
└── Development Timeline Synchronization
            """, language='python')
    with cols[2]:
        with st.container():
...
├── Revenue Stream Analysis (All Verticals)
├── Cross-Investment Opportunity ID
├── Infrastructure ROI on Property Values
...
Violation Detection → Revenue Optimization → Budget Allocation → HR Resource Assignment 
→ Hearing Management → Decision Support → Enforcement → Compliance Tracking 
→ Revenue Collection
    """, language='json')
...
def dashboard_page():
    ...
    with cols[0]:
        ...
        st.metric(label="Compounding Fees (Annual)", value="₹12.5 Cr")
    ...
    with cols[2]:
        st.metric(label="Property Sales (YTD)", value="₹126 Cr")
...
def benefits_page():
    ...
    cols = st.columns(4)
    with cols[0]:
        glass_card(lambda: st.markdown("""<h3 style="color:#22c55e;">Financial</h3><ul style="font-size:0.9rem;"><li>40% revenue increase</li><li>25% property premium</li><li>35% cost savings</li><li>₹12.5 Cr new revenue</li></ul>""", unsafe_allow_html=True))
    with cols[1]:
